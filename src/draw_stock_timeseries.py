...
def plot_timeseries(file, d, a):
    ...
    # KdqTree = pd.read_csv(f'{root}/results/exp_2023_ijcai/{file}_binned/full-1/KdqTree_bi_predictions_{file}.csv', delimiter=",", header=None).iloc[:, 0].tolist()
    ...
    if events:
        if mix or time_category == 'hourly':
        ...
        for j in range(len(insert_starts)):
            plt.fill_between(np.linspace(insert_starts[j], insert_ends[j]), ylim[0], ylim[1], color='gray', hatch='///', alpha=0.2)
            plt.fill_between(np.linspace(insert_ends[j], insert_ends[j] + 5*d), ylim[0], ylim[1], color='purple', hatch='///', alpha=0.1)
        ...
        #plt.legend(handles=all_handles, labels=labels, loc='upper left', bbox_to_anchor=(1.005, 1.4), prop={'size': 12})
        #plt.subplots_adjust(hspace=0.8)

    else:
        handles, labels = plt.gca().get_legend_handles_labels()
        plt.legend(handles=handles, labels=labels, loc='upper left', bbox_to_anchor=(1.005, 1.07),
                   prop={'size': 16})
        plt.subplots_adjust(hspace=0.8)

    plt.savefig(f'../data/ticker_data_plots/{file}_relocation.png')
    # plt.show()
    print(f'time series is saved in data/ticker_data_plots/{file}_relocation.png')
    plt.close()


if __name__ == '__main__':

    plot_timeseries('Open_hourly', 30, 21)
    # plot_timeseries('BMW.DE_daily', 50, 35)
    # plot_timeseries('BMW.DE_hourly', 30, 21)
    # plot_timeseries('Open_hourly(events2)', 30, 21)
