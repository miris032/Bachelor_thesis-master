...
def drawPlot(file, binning, d, a):
    ...
    for i, value in enumerate(timeseries_predictions):
        if value == 1:
            ax1.axvline(x=i, color='red', linestyle='-', linewidth=0.8, alpha=0.01)

    HDDDM = [2799, 4199, 11199, 11899, 13999, 14699, 16099]
    for pos in HDDDM:
        ax1.axvline(x=pos, color='blue', linestyle='--', linewidth=0.8, alpha=0.5)
    ...
    ax2.set_yticks([0, 1])
    fig.legend(loc='upper left', bbox_to_anchor=(0.9, 0.73))
...
if __name__ == '__main__':
    drawPlot('dataset1-timeSeries_after_filling', '_binned', 200, 140)
